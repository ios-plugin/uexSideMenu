...
- (id)initWithItems:(NSArray *)items {
    ...
    if (self) {
        ...
        _menuPosition = LDSideMenuPositionRight;
    }
...
- (void)showItem:(UIView *)item {
    ...
    if (self.menuIsVertical) {
        position.x += self.menuPosition == LDSideMenuPositionRight ? - self.menuWidth : self.menuWidth;
        
        [self animateLayer:item.layer
               withKeyPath:@"position.x"
                        to:position.x];
    } else {
        position.y += self.menuPosition == LDSideMenuPositionTop ? self.menuHeight : - self.menuHeight;
...
- (BOOL)menuIsVertical {
    if (self.menuPosition == LDSideMenuPositionLeft || self.menuPosition == LDSideMenuPositionRight)
        return YES;
...
- (void)layoutSubviews {
    ...
    if (self.menuIsVertical) {
        x = self.menuPosition == LDSideMenuPositionRight ? self.superview.frame.size.width : 0 - self.menuWidth;
        ...
        y = self.menuPosition == LDSideMenuPositionTop ? 0 - self.menuHeight : self.superview.frame.size.height;
    }
